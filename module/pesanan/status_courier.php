<?php
	...
	$query = mysqli_query($koneksi, "SELECT status_courier FROM pesanan WHERE pesanan_id='$pesanan_id'");
	$row=mysqli_fetch_assoc($query);
	$status_courier = $row['status_courier'];
...
<form action="<?php echo BASE_URL."module/pesanan/action.php?pesanan_id=$pesanan_id"; ?>" method="POST">
	...
	<div class="element-form">
		<label>Status</label>
		<span>
			<select name="status_courier">
				<?php
					
					
					foreach($arrayStatusCourier AS $key => $value){
						if($status == $key){
